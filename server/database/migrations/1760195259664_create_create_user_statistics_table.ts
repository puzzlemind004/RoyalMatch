...
export default class extends BaseSchema {
  ...
  async up() {
    this.schema.createTable(this.tableName, (table) => {
      ...
      table.decimal('win_rate', 5, 2).notNullable().defaultTo(0.0) // ex: 45.67%
      ...
      table.decimal('tricks_won_average', 5, 2).notNullable().defaultTo(0.0)

      // Scores
      table.integer('best_score').notNullable().defaultTo(0)
      table.integer('total_points').notNullable().defaultTo(0)
      table.decimal('average_score', 8, 2).notNullable().defaultTo(0.0)
